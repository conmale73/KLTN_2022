...
spotify = spotipy.Spotify(
    client_credentials_manager=SpotifyClientCredentials(
        client_id="721d6f670f074b1497e74fc59125a6f3",
        client_secret="efddc083fa974d39bc6369a892c07ced",
    )
)
...
def thumbnail(songId):
    ...
    driver = webdriver.Chrome(
        service=Service(os.environ.get("CHROMEDRIVER_PATH")), options=op
    )
...
def search():
    ...
    ytmusic = (
        YTMusic(location=region, language=language)
        if language != None
        else YTMusic(location=region)
    )
    if filter != None:
            ...
            "artist": [
                {
                    ...
                    "id": result["uploaderUrl"].replace("/channel/", ""),
                }
            ]
        }
    )
    data.update(
        {"thumbnails": [{"height": 720, "width": 1280, "url": result["thumbnailUrl"]}]}
    )
                    ...
                    "thumbnails": [
                        {
                            "height": 720,
                            "width": 1280,
                            "url": f"https://i.ytimg.com/vi/{videoId}/maxresdefault.jpg",
                        }
                    ]
                }
            )
...
def get_song_metadata():
    ...
    search_keyword_for_spotify_id = (
        re.sub(r"\(feat\..*?\)", "", title).strip() + " " + artist
    )
    search_keyword_for_spotify_id = re.sub(
        r"[^\w\s]", "", search_keyword_for_spotify_id
    )
...
def get_explore_mood():
    ...
    for item in items_mood:
        title = nav(
            item, ["musicNavigationButtonRenderer", "buttonText", "runs", 0, "text"]
        ).strip()
        endpnt = nav(
            item,
            [
...
def get_explore_playlist():
    ...
    response_1 = ytmusic._send_request(
        "browse", {"browseId": endpoint, "params": params}
    )
    with open("logs.json", "w") as f:
        json.dump(response_1, f, indent=4)
    header = nav(
        response_1, ["header", "musicHeaderRenderer", "title", "runs", 0, "text"]
    )
    items = nav(
        response_1,
        [
        ...
        for item_content in item_contents:
            ...
            if "musicResponsiveListItemRenderer" in item_content:
                ...
                for i in range(
                    ...
                    subtitle += nav(
                        item_content,
                        ["musicTwoRowItemRenderer", "subtitle", "runs", i, "text"],
                    )
                ...
                if "browseEndpoint" in nav(
                    item_content, ["musicTwoRowItemRenderer", "navigationEndpoint"]
                ):
                    playlistBrowseId = nav(
                        item_content,
                        [
                            "musicTwoRowItemRenderer",
                            "navigationEndpoint",
    ...
    result = {
        "header": header,
        "itemsSong": itemsSong,
        "itemsPlaylist": items_playlist_list,
    }
    return convert_to_json(result)
...
def get_explore_mood_playlist():
    ...
    for item in items:
        ...
        for content in contents:
            ...
            subtitle = (
                nav(mTRIR, ["subtitle", "runs", 0, "text"])
                + nav(mTRIR, ["subtitle", "runs", 1, "text"])
                + nav(mTRIR, ["subtitle", "runs", 2, "text"])
            )
            playlistBrowseId = nav(
                mTRIR, ["navigationEndpoint", "browseEndpoint", "browseId"]
            )
            ctd = {
                "title": title,
                "subtitle": subtitle,
